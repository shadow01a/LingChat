...
def start_tts_software():
    ...
    thread = threading.Thread(
        target=_run_tts_process, args=(tts_path,), daemon=True, name="TTS-Auto-Start"
    )
    thread.start()
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            process = subprocess.Popen(
                [str(tts_path)], cwd=cwd, creationflags=subprocess.CREATE_NEW_CONSOLE
            )
            ...
            """
            process = subprocess.Popen(["osascript", "-e", apple_script], cwd=cwd)
        else:
